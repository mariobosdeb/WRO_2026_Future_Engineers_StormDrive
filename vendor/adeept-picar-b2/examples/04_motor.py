...
def Motor(channel,direction,motor_speed):
  ...
  if direction == -1:
    speed = -speed
  if channel == 1: motor1.throttle = speed
  elif channel == 2: motor2.throttle = speed
  elif channel == 3: motor3.throttle = speed
  elif channel == 4: motor4.throttle = speed
...
if __name__ == '__main__':
  try:
    for i in range(10):
      speed_set = 110
      for ch in (1,2,3,4): Motor(ch, 1, speed_set)
      print("Forward")
      time.sleep(2)
      for ch in (1,2,3,4): Motor(ch, -1, speed_set)
      print("Backward")
